...
const COUNTRY_CONFIG = {
  countryName: 'Ireland',
  sourceCSVPath: 'C:\\Users\\robbu\\Desktop\\WASSP\\FILE EXCEL\\IRELAND.csv',
...
async function importIreland() {
  ...
  console.log(`Parsed ${records.length} records from Ireland CSV\n`);
  ...
  // Generate codes (starting from IRL0010)
  console.log('Generating codes...');
  normalized.forEach((record, idx) => {
    if (!record.code) {
      record.code = `IRL${String(10 + idx).padStart(4, '0')}`;
    }
  });
  console.log(`✓ Generated codes: IRL0010 to IRL${String(10 + normalized.length - 1).padStart(4, '0')}\n`);
  ...
  const exportPath = `ireland_associations_normalized.csv`;
  exportCSV(normalized, exportPath);

  // Generate audit report
  const auditPath = `ireland_import_audit.log`;
  generateAuditReport({
