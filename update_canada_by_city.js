...
async function updateCanada() {
  console.log('\n╔═══════════════════════════════════════════╗');
  console.log('║  UPDATE CANADA BY CITY                    ║');
  console.log('║  WASP IS FAIR AND DEMOCRATIC              ║');
  console.log('╚═══════════════════════════════════════════╝\n');

  // Read the CSV
  const csvPath = 'canada_cities_extracted.csv';
  ...
  console.log(`Found ${records.length} Canada associations\n`);
  ...
  for (const [city, codes] of Object.entries(cityMap)) {
    ...
    try {
      const coords = await geocodeCity(city, 'Canada');
