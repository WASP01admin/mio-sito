...
/**
 * UPDATE USA SHEET1 BY CITY
 * Geocode cities for remaining USA associations (Sheet1 batch)
 */
...
async function updateUSASheet1() {
  console.log('\n╔═══════════════════════════════════════════╗');
  console.log('║  UPDATE USA SHEET1 BY CITY                ║');
  console.log('║  WASP IS FAIR AND DEMOCRATIC              ║');
  console.log('╚═══════════════════════════════════════════╝\n');

  // Read the CSV
  const csvPath = 'usa_sheet1_cities.csv';
  ...
  console.log(`Found ${records.length} USA Sheet1 associations\n`);
  ...
  for (const [city, codes] of Object.entries(cityMap)) {
    ...
    try {
      const coords = await geocodeCity(city, 'United States');
  ...
  if (updates.length > 0) {
    ...
    console.log(`\n  ✓ Updated ${updated}/${updates.length} (${progress}%)`);

    console.log('\n╔═══════════════════════════════════════════╗');
    console.log('║  USA SHEET1 COMPLETE ✓                   ║');
    console.log('╚═══════════════════════════════════════════╝\n');
