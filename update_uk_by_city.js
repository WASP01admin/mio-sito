...
async function updateUK() {
  console.log('\n╔═══════════════════════════════════════════╗');
  console.log('║  UPDATE UK BY CITY                        ║');
  console.log('║  WASP IS FAIR AND DEMOCRATIC              ║');
  console.log('╚═══════════════════════════════════════════╝\n');

  // Read the CSV
  const csvPath = 'uk_cities_extracted.csv';
  ...
  console.log(`Found ${records.length} UK associations\n`);
  ...
  for (const [city, codes] of Object.entries(cityMap)) {
    ...
    try {
      const coords = await geocodeCity(city, 'United Kingdom');
