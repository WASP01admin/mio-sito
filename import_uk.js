...
/**
 * IMPORT UNITED KINGDOM ASSOCIATIONS
...
const COUNTRY_CONFIG = {
  ...
  sourceCSVPath: 'C:\\Users\\robbu\\Desktop\\WASSP\\FILE EXCEL\\UK.csv',
...
async function importUK() {
  ...
  console.log(`Parsed ${records.length} records from UK CSV\n`);
  ...
  // Clean up invalid coordinates (DMS format or malformed)
  console.log('Cleaning up coordinates...');
  let invalidCoords = 0;
  normalized.forEach((record) => {
    // Check if coordinates are in invalid format (contain degrees symbol, apostrophe, etc.)
    if (record.lat || record.lng) {
      ...
      if (latStr.includes('°') || latStr.includes("'") || latStr.includes('E') || latStr.includes('N') ||
        ...
        // Invalid coordinate format - clear it
        record.lat = null;
        record.lng = null;
        invalidCoords++;
      }
  ...
  if (invalidCoords > 0) {
    console.log(`✓ Cleaned up ${invalidCoords} records with DMS/invalid coordinate formats\n`);
  }

  // Generate codes (starting from GBR0010)
  console.log('Generating codes...');
  normalized.forEach((record, idx) => {
    if (!record.code) {
      record.code = `GBR${String(10 + idx).padStart(4, '0')}`;
    }
  });
  console.log(`✓ Generated codes: GBR0010 to GBR${String(10 + normalized.length - 1).padStart(4, '0')}\n`);
  ...
  const exportPath = `uk_associations_normalized.csv`;
  exportCSV(normalized, exportPath);

  // Generate audit report
  const auditPath = `uk_import_audit.log`;
  generateAuditReport({
